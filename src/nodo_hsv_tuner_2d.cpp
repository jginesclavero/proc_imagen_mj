 ...
 *
 *
 * Programa de prueba de filtrado de imagen en HSV
	...
	void imageCb(const sensor_msgs::Image::ConstPtr& msg)
	{
		...
		for(int i = 0; i <height; i++ )
			for(int j = 0; j <width; j++ )
			{
				...
				if(!((hsv.data[posdata] >= hsvValues_[channel_].hsv[IDX_h]) && (hsv.data[posdata] <= hsvValues_[channel_].hsv[IDX_H]) &&
						(hsv.data[posdata+1] >= hsvValues_[channel_].hsv[IDX_s]) && (hsv.data[posdata+1] <= hsvValues_[channel_].hsv[IDX_S]) &&
						(hsv.data[posdata+2] >= hsvValues_[channel_].hsv[IDX_v]) && (hsv.data[posdata+2] <= hsvValues_[channel_].hsv[IDX_V])
				))
				{
